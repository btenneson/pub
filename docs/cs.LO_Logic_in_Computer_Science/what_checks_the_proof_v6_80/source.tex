...
\usepackage[letterpaper,margin=1in,headheight=18pt]{geometry}
...
\hypersetup{
  ...
  pdfsubject={AMLD architecture, verified banks, FUTUREBANK counterfactual search, proof search, verification, and formalized self-description},
...
% Running heads: publication shorthand | author | current chapter/section.
% Chapter marks seed the right mark; numbered sections then replace it, so chapter-opening
% pages still have a meaningful contextual header.
\renewcommand{\chaptermark}[1]{\markboth{\thechapter\quad #1}{\thechapter\quad #1}}
\renewcommand{\sectionmark}[1]{\markright{\thesection\quad #1}}
\newcommand{\AMLDHeaderLeft}{\footnotesize\textsc{WCTP/AMLD}}
\newcommand{\AMLDHeaderCenter}{\footnotesize Brian Tenneson}
\newcommand{\AMLDHeaderRight}{\footnotesize\truncate{0.40\headwidth}{\nouppercase{\rightmark}}}
\newcommand{\AMLDPageFooter}{\footnotesize Page~\thepage}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\AMLDHeaderLeft}
\fancyhead[C]{\AMLDHeaderCenter}
\fancyhead[R]{\AMLDHeaderRight}
\fancyfoot[C]{\AMLDPageFooter}
...
\normalsize Textbook Version 6.80\\[1.15em]
\begin{minipage}{0.88\textwidth}
\footnotesize
\textbf{Elementary idea.} A computer can suggest useful mathematical steps, but a suggestion is not a proof. This book studies a system in which several search roles try different ways to settle a statement. Verified facts go into a shared \textbf{BANK}; imagined possibilities stay separate in a \textbf{FUTUREBANK}. A checker decides what is allowed to count as established mathematics. In short: \emph{imagine freely; verify rigidly}.

Think of the BANK as a shared notebook containing the starting assumptions and results that have actually passed inspection. The FUTUREBANK is a scratchpad for ``what if?'' routes that may be worth exploring. One role tries to prove the target, another to refute it, another investigates independence, and another checks for contradiction. They may share verified discoveries, but guesses stay guesses until a certificate is checked. If no decisive certificate is found, the honest answer is \textsf{UNKNOWN}.\par
\end{minipage}}
\author{Brian Tenneson, M.A., M.S.\\[0.35em]
\small M.A. Mathematics, San Francisco State University\\
\small M.S. Applied Data Science, Bay Path University\\[0.45em]
\small \href{mailto:btenneson2301@baypath.edu}{btenneson2301@baypath.edu}\\
\small \href{https://btenneson2301.substack.com}{btenneson2301.substack.com}}
\date{August 22, 2026}
...
\begin{center}
\small
Copyright \textcopyright\ 2026 Brian Tenneson. All rights reserved.\\[0.8em]
\textit{What Checks the Proof? A Textbook Introduction to Automated Mathematical Logic Deciders}\\
Textbook Version 6.80\\[0.8em]
\href{mailto:btenneson2301@baypath.edu}{btenneson2301@baypath.edu}\\
\href{https://btenneson2301.substack.com}{btenneson2301.substack.com}
\end{center}
\vspace*{\fill}
\clearpage

\chapter*{Elementary Foreword}
\markboth{Elementary Foreword}{Elementary Foreword}
\addcontentsline{toc}{chapter}{Elementary Foreword}

This book asks a question that does not require any formal logic to understand:
\begin{quote}
\emph{When a machine says that it has proved something, what checks the proof?}
\end{quote}
A computer can be very good at proposing ideas without every proposal being correct. That is not a defect. Human mathematicians also work by trying examples, making guesses, exploring side paths, and asking ``what if?'' The important distinction is between \emph{searching for an answer} and \emph{certifying an answer}. This book studies an architecture that tries to make that distinction explicit. Search may be flexible and imaginative; verification is deliberately strict. The recurring slogan is \textbf{Imagine freely; verify rigidly.}

\section*{The BANK and the FUTUREBANK}
Imagine several careful mathematical explorers working around one shared notebook. Whenever one of them establishes a useful result and the result passes the required check, it can be written into the notebook for everybody to use. That shared notebook is the elementary picture behind the \textbf{BANK}.

The BANK begins with the assumptions that the problem has declared in advance. The book usually calls this starting set $\Gamma$. At an elementary level, $\Gamma$ can simply be read as ``the rules and facts we agreed to start with.'' After that starting point is fixed, derived results are admitted only when the appropriate checker accepts their certificates. A useful idea does not enter merely because it is plausible, elegant, or produced by a sophisticated program.

Now give the same explorers a scratchpad. On it they may write, ``Suppose this lemma were available,'' ``What happens if we pursue the negation?'' or ``This branch looks more promising than that one.'' Those notes can guide later work without being treated as established mathematics. The \textbf{FUTUREBANK} plays that role. It represents possible proof futures: deductions, hypothetical lemmas, or branch histories that may be worth exploring.

The distinction is simple but central. The BANK represents what has actually survived checking. The FUTUREBANK represents what might happen. A FUTUREBANK idea can eventually lead to a real BANK deposit, but only by crossing the verification boundary.

\section*{Four ways to try to settle one statement}
Ordinary automated theorem proving is often one-sided: give the machine a target and ask it to prove the target. An \emph{Automated Mathematical Logic Decider}, abbreviated \AMLD, organizes a broader settlement search around four principal roles.

The prover $P$ tries to prove the target. The refuter $R$ tries to prove its negation. The independence role $I$ looks for a genuine metamathematical certificate that the target and its negation are both unprovable from the declared assumptions. The contradiction role $C$ checks whether the admitted background itself leads to contradiction.

These are search roles, not four separate owners of mathematics. If $R$ discovers a useful ordinary lemma while pursuing a refutation, that lemma may later help $P$. The shared BANK makes such reuse possible. Provenance records where a result came from, while the certificate says what the result actually establishes.

A tiny example shows the pattern. Suppose the starting information contains $p$ and ``if $p$, then $q$.'' A prover proposes $q$ by modus ponens. If the checker confirms that the inference is valid in the declared environment, $q$ can enter the BANK as a derived result. The example is intentionally easy. It makes the architecture visible: there are admitted premises, a proposed step, a checker, and a verified deposit.

\section*{The verifier and the honest answer UNKNOWN}
The verifier is intentionally less adventurous than the search machinery. It does not need to decide which idea is beautiful or which path is creative. Given the formal environment and a proposed certificate, its job is to answer a narrower question: does this certificate satisfy the declared rules for acceptance?

That narrowness is a strength. The search policy may improve, learn from failure, change priorities, or use new forms of planning without changing what counts as a valid proof. The architecture therefore separates \emph{where to look} from \emph{what counts once found}.

The verifier is not an oracle for all mathematical truth. Everything is relative to a declared setting: language, assumptions, inference rules, certificate formats, and checking procedures. Nor does a long failed search prove that the target is false or independent. If the machine exhausts its present resources without producing a decisive certificate, the honest computational output is \textsf{UNKNOWN}. A timeout describes the computation; it is not automatically a theorem about the mathematics.

\section*{Why imagination belongs here}
The word \emph{imagination} can sound strange when applied to a proof system, but the intended meaning is concrete. A chess engine can consider moves that have not been played, examine possible replies, and use those hypothetical futures to decide where to search. It does not confuse an imagined move with a move already made.

The FUTUREBANK gives proof search a similar counterfactual workspace. A \textbf{COMPASS} can rank possible directions, while a controller decides how to distribute effort. The machine can learn from rejected attempts, notice unproductive regions, or temporarily explore what would follow from a hypothetical lemma. None of this makes the hypothetical lemma true. It only changes what the system chooses to investigate next.

Later chapters also discuss formalized self-description. The elementary idea is modest: a machine can represent facts about its own search, such as which branch it is exploring, which role has recently succeeded, or whether fallback search is receiving attention. Such information can guide control. It still does not redefine proof. ``I have searched here for a long time'' may be useful information about the computation; it does not imply ``therefore the target is true.''

\section*{The map for the rest of the book}
The technical chapters turn these pictures into definitions, state spaces, theorems, pseudocode, worked examples, exercises, and research conjectures. The notation becomes more precise, but a reader who keeps five elementary ideas in mind already has the basic map:
\begin{enumerate}
\item Search may try things that are not yet proved.
\item The BANK contains admitted starting material and verified results.
\item The FUTUREBANK contains represented possibilities, not automatically facts.
\item The roles $P,R,I,C$ pursue proof, refutation, independence, and contradiction while sharing certified work.
\item The verifier is the gate; without a valid terminal certificate, the honest answer is \textsf{UNKNOWN}.
\end{enumerate}

The proposal is therefore not that difficult mathematics becomes easy simply by giving software new component names. It is an architectural discipline: keep verified memory separate from speculative possibility, let several settlement roles reuse certified work, preserve where results came from, allow planning to guide search, and keep the acceptance standard explicit. Everything that follows develops those ideas more carefully.
...
Two earlier program documents provide historical context for the present architecture: the August 2026 automated-logical-decider research supplement and the July 2026 \emph{Depths of a Simulation \& Formalized Self-Awareness} manuscript \cite{TennesonALD2026,TennesonDepths2026}. They are cited as prior program documents, not as substitutes for the independently cited logic, model-theory, automated-reasoning, security, and nonstandard-analysis literature.

The guiding sentence of the entire book is:
\begin{quote}
\textbf{Imagine freely; verify rigidly.}
\end{quote}
The BANK remembers what has actually survived. The FUTUREBANK represents what might happen. The COMPASS points. The controller plans. Formal self-description may reason about that process. The declared environment admits the seed assumptions $\Gamma$; after that seed is fixed, the verifier is the gate that decides which derived claims may enter verified mathematical memory.

\chapter*{Opening roadmap}
\markboth{Opening roadmap}{Opening roadmap}
...
\begin{roadmap}
We begin with the simplest distinction in the book: proving one target is not the same task as settling its status. We define four certificate-indexed settlement conclusions plus the operational output UNKNOWN, explain why timeouts are not mathematical evidence, and identify exactly what the four search roles are trying to certify.
\end{roadmap}
...
\begin{quote}
What positive certificate, if any, settles the logical or metamathematical status of $c$ relative to $\Gamma$?
\end{quote}
...
\]
The BANK did not merely store the answer; its dependency and provenance fields preserve the route by which the answer became admissible.
...
Let $\Gamma=\{a, a\to b, b\to d\}$ and target $d$. Write the initial bank records and a two-deposit history (three BANK states $\Bank_0,\Bank_1,\Bank_2$) that ends with $d$. Which records are admitted premises and which are derived?
\end{exercise}
...
k=(a,(t,\kappa)),
\]
where $a$ says \emph{who submitted it}, $t$ says \emph{what kind of certificate it claims to be}, and $\kappa$ is the certificate body. For this abstract interface, $\kappa$ is understood to contain or bind every truth-relevant field needed by the declared checker---in particular the claimed conclusion, scope, dependency references, and proof/metacertificate data as appropriate to kind $t$. Implementations may store those fields separately while passing the same semantic content to the checker.
...
\]
A software wrapper may receive the full proposal $k=(a,s)$, record $a$ as provenance, resolve the admissible dependencies named by $s$, and pass the truth-relevant semantic content to the intrinsic checker. In a concrete implementation the fields making up $s$ may be stored separately---for example as semantic kind, claim, scope, dependencies, and certificate body---but the submitting-role tag remains outside the intrinsic truth predicate.
...
The TPTP/SZS ecosystem contains a mature vocabulary for automated-reasoning outcomes, distinguishing statuses such as theoremhood, counter-satisfiability, satisfiability, and unsatisfiability, and it supports proof/model output conventions \cite{TPTPQuick,TPTPLanguage}. The AMLD model-pair pattern should therefore be understood as building on established proof- and model-producing reasoning practice, not as if model-based negative evidence had no precedent.
...
\]
Let $\StateSpace$ denote the declared set of admissible tuples of this form---that is, tuples satisfying the architecture's typing, scope, environment, and state-invariant requirements---so $\State_m\in\StateSpace$. The individual coordinate spaces may vary by implementation; the notation $\StateSpace$ does not assume that every arbitrary tuple is admissible.
...
\item submit it to $V$;
\item if accepted, add the resulting typed, scoped record to the BANK;
...
The distinction between \emph{intermediate} and \emph{terminal} accepted records is important. The BANK can grow for a long time without the target being settled.
...
The BANK is shared, but a metamathematical independence statement is not automatically usable as an object-level theorem inside the theory whose independence is being discussed. A scoped hypothetical lemma is not automatically global. A contradiction certificate changes the interpretation of the whole run in explosive logics.
...
The BANK can therefore be understood as a proof-carrying shared memory rather than a bag of formulas. In the record-set model used here, separately accepted derivations of the same displayed formula may remain distinct records with distinct receipts and origins; a formula-level retrieval index may still present a canonicalized view without erasing that ledger provenance.
...
\[
L_{\Bank}\subseteq \mathrm{RecordID}\times\N\times\mathcal P_{\mathrm{fin}}(\mathrm{RecordID})
\]
...
Let $\operatorname{Succ}(x)$ be the represented successors of proof state $x$. If $h$ is a history, let $\operatorname{end}(h)$ be its final state. Starting from the current history $h_0$, define the generation layers
\[
\mathcal G_0=\{h_0\},
\]
and recursively let $\mathcal G_{d+1}$ contain one-step extensions of histories in $\mathcal G_d$. The depth-$d$ FUTUREBANK is
\[
\Future_d=\bigcup_{j=0}^{d}\mathcal G_j.
\]
...
A literal history tree preserves provenance automatically but can duplicate enormous downstream computation. A DAG can share equivalent future states, but only if the equivalence relation preserves everything relevant to future search and verification.

For clarity, the implementation pseudocode later uses single-parent history nodes as its correctness-first baseline. Computation sharing is then an optimization layered on top of those histories, not a reason to erase them. A FUTUREBANK implementation may share downstream work while separately retaining whatever path multiplicity, incoming transition data, or ancestry the task needs.
...
\end{itemize}
One common trust path is
\[
\Spec\longrightarrow\Loc\longrightarrow V\longrightarrow\Bank,
\]
where the local-check stage is optional: a speculative object may also be submitted directly to the final verifier when its certificate format permits. The arrows indicate increasing certification, not increasing subjective confidence.
...
This proposition does not say that every promising imagined branch can be backfilled, nor does it claim that its stated dependency order is the only possible route. It gives a clean sufficient condition under which a speculative finite dependency DAG can be promoted into verified mathematics.
...
\begin{roadmap}
This Part adds formal self-description without confusing it with imagination. We begin with Tegmark's notion of self-aware substructures as an intellectual influence, then define a checked architectural scale for AMLD self-description and explain why iterated provability is not an adequate level counter. Finally we show that self-description may steer search while leaving proof semantics unchanged.
\end{roadmap}
...
The relevance here is structural. A formal machine may contain a representation of itself and may reason about that representation. Once this possibility is admitted, one can ask precise questions: Which self-descriptions can it check? Which formal provability claims can it establish in a declared theory? Which facts about its own current search state can it safely use as controller input?
...
Let $\mathsf A$ be an AMLD with a formal name or code $\langle\mathsf A\rangle$ available in a declared metatheory $T$. A minimal static self-description may contain a formula such as
\[
\AMLD(\langle\mathsf A\rangle),
\]
together with machine-checkable facts identifying the architecture version, verifier version, and role set. The point is not that the string is intrinsically profound. The point is that the formal environment can refer to the machine itself as an object of reasoning and can distinguish a checked self-description from an unchecked assertion about itself.

\section{Why iterated provability is not used as the level scale}
Suppose the declared metatheory $T$ is recursively axiomatized strongly enough to arithmetize syntax and has a standard provability predicate $\Prov_T$. A tempting definition would set $R_0(\mathsf A)=\AMLD(\langle\mathsf A\rangle)$ and then iterate
\[
R_{n+1}(\mathsf A)=\Prov_T(\ulcorner R_n(\mathsf A)\urcorner).
\]
That notation is useful in provability logic, but it is a poor measure of increasing architectural self-description. For standard arithmetized provability predicates satisfying the usual derivability conditions, if $T$ proves a sentence $A$, then $T$ also proves the corresponding assertion $\Prov_T(\ulcorner A\urcorner)$. Repeating the operator can therefore add syntactic layers without adding a genuinely new self-model. Reflection principles also interact with G\"odelian and L\"obian limitations and require the object theory, metatheory, coding, and provability predicate to be specified with care \cite{SEPProvability}. This edition therefore does \emph{not} identify ``one more provability box'' with ``one more level of self-awareness.''

\section{A checked self-description scale}
Fix a metatheory $T$ capable of coding $\mathsf A$, the current search state $\State_m$, and the receipts produced by a declared family of self-description checkers. A self-description counts at any level only when its checker verifies the claimed relationship between the descriptor and the actual coded machine/state. The following scale is an architectural convention for this book, not a universal hierarchy of minds.

\begin{definition}[Formal self-description levels]
Relative to a declared architecture and self-description checker family:
\begin{itemize}
\item \textbf{Level 0}: no nontrivial checked self-description of the designated kind is available to the controller.
\item \textbf{Level 1}: checked \emph{static} self-description is available: for example, the machine can identify its own code or version, the roles it implements, and the verifier environment it is using.
\item \textbf{Level 2}: Level 1 is available and checked \emph{dynamic} self-state facts can also be represented and used by the controller, such as the current BANK identifier, selected FUTUREBANK dependencies, verifier-history summaries, resource counters, or whether complete fallback has received its reserved steps.
\item \textbf{Level $n$} for finite $n\ge3$: an implementation-specific extension adds a further bounded layer of checked metacontrol description, such as representing and evaluating how a lower-level controller or self-model would change under candidate search actions. The exact schema must be declared; the symbol $n$ alone does not determine one.
\item \textbf{Level $\omega$}: every declared finite level schema can in principle be instantiated and checked. This means ``all finite levels are supported,'' not that the machine stores a completed infinite object or performs infinitely many operations in a finite run.
\end{itemize}
\end{definition}

The scale measures formalized self-description and metacontrol capacity. It does not measure proof depth, intelligence, subjective consciousness, FUTUREBANK size, or chess strength. Level 2 is singled out later only as a plausible \emph{engineering} sweet spot: it exposes useful current-state information to control without automatically incurring the cost of deeper metareasoning. Whether that tradeoff is favorable is an empirical conjecture, not a theorem.
...
Consider any class of computational search systems containing both (i) a system with nontrivial counterfactual/FUTUREBANK search but no checked Level-1 self-description available to its controller, and (ii) a tagged system with a checked Level-1 self-description but only a constant or nonbranching search policy. Within such a class, nontrivial FUTUREBANK search does not imply Level-1 formal self-description, and Level-1 formal self-description does not imply nontrivial FUTUREBANK search.
\end{proposition}
\begin{proof}
...
Call an AMLD \emph{FUTUREBANK-reflective} relative to an architecture description if its formal language can name selected search components and it can obtain checker-accepted statements about some of them, such as:
\begin{itemize}
...
\end{itemize}
These are examples of Level-2-style dynamic self-description. They are architecture-relative facts, and they should not be confused with unrestricted claims about the machine's own soundness or truth.
...
\[
S:\StateSpace\times\N\longrightarrow\Cert\cup\{\textsf{NONE}\}.
\]
...
\begin{roadmap}
Corollary~\ref{cor:unknowncomputational} says that a finite-resource AMLD may honestly return \textsf{UNKNOWN} even when the metatheoretic status of a standard sentence is fixed. This chapter studies an idealized unlimited nonstandard-natural bound. The formal results require only an elementary extension of the standard structure of arithmetic; first-order compactness and the elementary diagram supply such an extension together with an element larger than every standard natural. A richer nonstandard-analysis enlargement is needed only if one wants to speak literally about internal hyperfinite sets, hyperfinite BANKs, or an internal run of hyperfinite length. Keeping those two frameworks separate is essential. We then prove coverage of every standard finite PA proof, derive an external classification of standard PA sentences, and explain why none of this yields an ordinary computable decision procedure or an automatic finite independence certificate.
\end{roadmap}

\section{The nonstandard setting}
Let \(\N\) denote the standard first-order structure of arithmetic in the usual language of PA. The arithmetic results below use an elementary extension
\[
\N\preccurlyeq M.
\]
Here \(\N\preccurlyeq M\) means that the inclusion is elementary: every first-order formula with parameters from the embedded standard copy has the same truth value in \(\N\) and in \(M\). We reserve the star notation \({}^*\N\) for the later, optional full nonstandard-analysis framework. This notational separation prevents an elementary extension of arithmetic from being mistaken for an entire nonstandard universe. We first justify the existence of \(M\) with an element above every standard natural.

\subsection{Compactness supplies the unlimited horizon}\label{sec:compactness-hypernatural}
The first-order \emph{Compactness Theorem} says that a set \(T\) of first-order sentences has a model whenever every finite subset of \(T\) has a model. Enderton proves this standard formulation in Section~2.5 after the completeness theorem \cite{EndertonLogic}. Marker treats compactness in Section~2.1 and elementary embeddings and elementary diagrams in Section~2.3 \cite{MarkerModelTheory}.

Expand the language of arithmetic by adding a constant symbol \(c_a\) for every \(a\in\N\). Let
\[
\operatorname{ElDiag}(\N)
\]
denote the \emph{elementary diagram} of \(\N\): all sentences in this expanded language that are true in \(\N\) when each \(c_a\) is interpreted as \(a\). Add one further constant symbol \(H\), and consider
\[
T_H
=
\operatorname{ElDiag}(\N)
...
\begin{theorem}[Compactness existence of an unlimited nonstandard natural]\label{thm:compactness-unlimited}
The theory \(T_H\) has a model. Consequently there is a proper elementary extension of the standard natural-number structure containing an element greater than every standard natural number.
\end{theorem}
\begin{proof}
Take any finite subset \(T_0\subseteq T_H\). Only finitely many inequalities involving \(H\) occur in \(T_0\), say
\[
H>c_{n_1},\ldots,H>c_{n_k}.
\]
If no such inequality occurs, choose any ordinary interpretation for \(H\). Otherwise interpret every \(c_a\) by the corresponding standard natural \(a\) and choose an ordinary natural \(m>\max\{n_1,\ldots,n_k\}\) to interpret \(H\). Every elementary-diagram sentence in \(T_0\) is true by definition, and the finitely many displayed inequalities are true by the choice of \(m\). Thus every finite subset of \(T_H\) has a model.

By Compactness, \(T_H\) has a model \(M_H\). Since \(M_H\models\operatorname{ElDiag}(\N)\), the map
\[
a\longmapsto c_a^{M_H}
\]
is an elementary embedding of \(\N\) into the arithmetic reduct of \(M_H\). Identify \(\N\) with its image and write \(M\) for the resulting elementary extension. The interpretation of \(H\) satisfies
\[
H>n\qquad\text{for every standard }n\in\N.
\]
Therefore \(H\) is outside the embedded standard copy, the extension is proper, and \(H\) is an unlimited nonstandard natural.
\end{proof}

\begin{remark}[Existence is not execution]\label{rem:compactness-not-computation}
The theorem is an existence result in first-order model theory. It does not provide a standard natural number that can stand in for \(H\), a canonical computable presentation of \(M\), or an ordinary finite procedure that reaches stage \(H\). Compactness constructs no ``fantastically large ordinary integer.''
\end{remark}

\begin{remark}[Why the elementary diagram is used]\label{rem:eldiag}
If one added \(H>\overline n\) for every standard numeral merely to the complete theory \(\operatorname{Th}(\N)\), Compactness would give a nonstandard model elementarily equivalent to \(\N\). The present chapter wants the more specific relation \(\N\preccurlyeq M\). Naming every standard element and including the full elementary diagram ensures that the canonical map from the named standard copy into the compactness model is elementary. Marker gives the elementary-diagram machinery in Section~2.3 \cite{MarkerModelTheory}.
\end{remark}

\begin{warningbox}
The use of \(\operatorname{ElDiag}(\N)\) is a strong semantic hypothesis, not harmless notation. It packages the full first-order truth of the standard structure with named standard parameters. In particular, the resulting arithmetic extension satisfies \(M\models\operatorname{Th}(\N)\), not merely \(M\models\mathrm{PA}\). Compactness then proves that a suitable elementary extension exists, but it does not give an effective procedure for listing all of \(\operatorname{ElDiag}(\N)\), computing a presentation of \(M\), or deciding its satisfaction relation. The exact classification results later in the chapter rely on this elementarity; they are not consequences of ``being a nonstandard model of PA'' alone.
\end{warningbox}

\begin{remark}[Terminology: nonstandard natural versus hypernatural]\label{rem:hypernatural-terminology}
The compactness theorem above produces an elementary extension \(M\) of arithmetic. Its new elements are unambiguously \emph{nonstandard natural numbers}. In a full nonstandard-analysis framework equipped with a star map or enlargement, elements of \({}^*\N\) are conventionally called \emph{hypernaturals}. This chapter retains ``hypernatural'' in its title because that is the intended AMLD idealization, but the proofs of the arithmetic results need only \(M\). If a full NSA universe is later chosen, its natural-number sort may play the role of \(M\). Goldblatt gives a standard treatment of transfer, internal and external sets, hyperfinite sets, and nonstandard frameworks, while Keisler provides a complementary infinitesimal-calculus treatment \cite{GoldblattHyperreals,KeislerFoundations}.
\end{remark}

We now fix one such elementary extension
\[
\N\preccurlyeq M
\]
and one \(H\in M\) satisfying
\[
H>n\qquad\text{for every standard }n\in\N.
\]
We call such an \(H\) \emph{unlimited}. In the arithmetic framework, ``unlimited'' is a metatheoretic condition referring to the embedded standard copy; it is not a single first-order predicate of the language of arithmetic. In a full nonstandard-analysis enlargement, the collection of unlimited hypernaturals is external rather than internal. See Goldblatt's chapters on internal/external and hyperfinite entities \cite{GoldblattHyperreals}.

\begin{checkpoint}
The logical dependency is: \textbf{Compactness gives existence}; \textbf{the elementary diagram gives the elementary embedding}; \textbf{elementarity transfers first-order statements with standard parameters}; and \textbf{unlimitedness gives coverage of every standard finite proof code}. Elementarity supplies standard correctness; unlimitedness supplies the bound. Neither may be silently substituted for the other, and none of these steps gives a standard computer an executable value of \(H\).
\end{checkpoint}

\subsection{Two frameworks that must not be conflated}\label{sec:two-nsa-frameworks}
There are two legitimate readings of the discussion, but they support different vocabulary.
\begin{enumerate}
\item \textbf{Arithmetic elementary-extension framework.} Work only with \(\N\preccurlyeq M\), the PA proof predicate interpreted in \(M\), and a nonstandard bound \(H\in M\). This is sufficient for every theorem in this chapter. One should speak about truth in the elementary extension and bounded quantified formulas, not assume a whole universe of internal sets.
\item \textbf{Full nonstandard-analysis framework.} Work in a suitable nonstandard enlargement of a set-theoretic or superstructure universe. Then the natural-number sort contains hypernaturals, the interval \([0,H]\) is an internal hyperfinite set, and one may literally discuss internal functions and hyperfinite runs. Goldblatt treats internal functions and hyperfinite sets explicitly \cite{GoldblattHyperreals}.
\end{enumerate}
The second framework is useful for the AMLD picture, but it is stronger infrastructure than the arithmetic compactness theorem itself supplies. No theorem below depends on silently moving from the first framework to the second.

\subsection{The bounded proof-existence condition}
Fix a standard G\"odel coding of PA formulas and proofs. For a standard PA sentence \(\varphi\), let
\[
\Prf_{\mathrm{PA}}(p,\ulcorner\varphi\urcorner)
\]
be a standard arithmetical formula expressing that \(p\) codes a valid PA proof with conclusion code \(\ulcorner\varphi\urcorner\). The same formula is interpreted in the elementary extension \(M\).

For an unlimited \(H\in M\), define the bounded proof-existence condition
\[
B_H^M(\varphi)
\quad:\Longleftrightarrow\quad
M\models
(\exists p)\bigl[p\le H\wedge\Prf_{\mathrm{PA}}(p,\ulcorner\varphi\urcorner)\bigr].
\]
The superscript records the ambient elementary extension. This definition is model-theoretic: it states that a bounded first-order formula is true in \(M\). It does not presuppose that an external observer can execute a search of length \(H\).

\begin{proposition}[Why elementarity is essential]\label{prop:elementarity-essential}
The reverse direction in Theorem~\ref{thm:hyperproofequiv} below depends essentially on \(\N\preccurlyeq M\). Replacing \(M\) by an arbitrary nonstandard model of PA can make a nonstandard proof witness appear for a standard sentence that has no ordinary PA proof.
\end{proposition}
\begin{proof}
A standard example comes from the second incompleteness theorem. Assume PA is consistent and use the usual arithmetized consistency sentence \(\operatorname{Con}(\mathrm{PA})\). Then PA does not prove \(\operatorname{Con}(\mathrm{PA})\); hence PA together with \(\neg\operatorname{Con}(\mathrm{PA})\) is consistent and has a model \(A\) by completeness. In \(A\), the sentence \(\neg\operatorname{Con}(\mathrm{PA})\) asserts that some \(p\) satisfies the PA proof predicate for a contradiction. Since PA is externally consistent, no standard natural is such a proof code. In a model of PA the standard numerals form an initial segment of the arithmetic order, so any such witness \(p\in A\) is nonstandard and lies above every standard numeral. Taking \(H=p\), the analogue of \(B_H^A(\bot)\) holds even though there is no ordinary finite PA proof of \(\bot\). Enderton treats arithmetized provability and the second incompleteness theorem in Section~3.7 \cite{EndertonLogic}.
\end{proof}

Thus two ingredients have different jobs: the unlimited bound ensures that no \emph{standard} proof code is missed, while elementarity of the extension prevents spurious nonstandard witnesses for false standard proof-existence statements. This is why the chapter constructs an elementary extension of the standard structure rather than merely choosing an arbitrary nonstandard model of PA.

If \(M\) is realized as the natural-number sort \({}^*\N\) of a full nonstandard-analysis enlargement, then \(B_H^M(\varphi)\) may be pictured as the result of an internal hyperfinite exhaustive scan of
\[
[0,H]=\{p\in{}^*\N:0\le p\le H\}.
\]
We may call that optional idealized scan \(E_H\). The scan picture is pedagogically useful, but the formal results below are stated in terms of \(B_H^M\), so they do not depend on importing internal-set language into the arithmetic-only framework.

\section{Unlimited-bound proof equivalence}
\begin{theorem}[Unlimited-bound proof equivalence]\label{thm:hyperproofequiv}
Let \(H\in M\) be unlimited and let \(\varphi\) be a standard PA sentence. Then
\[
...
\end{theorem}
\begin{proof}
Suppose first that \(\mathrm{PA}\vdash\varphi\). Some ordinary finite PA proof of \(\varphi\) has a standard G\"odel code \(n\in\N\). Since \(H\) is unlimited, \(n<H\). The elementary embedding preserves the standard proof predicate on the standard parameter \(\ulcorner\varphi\urcorner\), so the same standard code witnesses \(B_H^M(\varphi)\).

Conversely, suppose \(B_H^M(\varphi)\). Then in particular
\[
M\models(\exists p)\,\Prf_{\mathrm{PA}}(p,\ulcorner\varphi\urcorner).
\]
The formula defining the PA proof predicate and the code \(\ulcorner\varphi\urcorner\) are standard. Since \(\N\preccurlyeq M\), elementarity gives
\[
\N\models(\exists n)\,\Prf_{\mathrm{PA}}(n,\ulcorner\varphi\urcorner).
\]
Hence an ordinary finite PA proof of \(\varphi\) exists. The bound \(p\le H\) is unnecessary in this reverse direction once any proof witness exists in the elementary extension.
\end{proof}

\begin{corollary}[Unlimited-bound coverage of standard positive certificates]\label{cor:hypercoverage}
Every standard finite \(P\)-, \(R\)-, or \(C\)-certificate for PA has a standard code below every unlimited \(H\). Thus the bounded condition \(B_H^M\) cannot miss an existing standard PA proof of \(\varphi\), \(\neg\varphi\), or \(\bot\).
\end{corollary}

\begin{corollary}[Standard positive-witness existence]\label{cor:positivehyperstandard}
If \(B_H^M(\varphi)\) holds for a standard PA sentence \(\varphi\), then an ordinary finite PA proof of \(\varphi\) exists. A witness to the bounded formula in \(M\) need not itself be standard.
\end{corollary}

The last sentence matters. Elementarity proves existence of some standard proof; it is not an algorithm that extracts that proof from a particular nonstandard witness. Once standard existence is known metatheoretically, ordinary enumeration of finite PA proofs eventually finds a standard witness, but no effective shortcut has been obtained merely from the nonstandard one.

\section{External classification, not an ordinary decider}
Assume for this section that PA is consistent. For each standard PA sentence \(\varphi\), define the metatheoretic classification
\[
D_H^M(\varphi)=
\begin{cases}
P,&\text{if }B_H^M(\varphi),\\
R,&\text{if }B_H^M(\neg\varphi),\\
I,&\text{if neither bounded condition holds.}
\end{cases}
\]
Here \(I\) means exactly
\[
\mathrm{PA}\nvdash\varphi
\quad\text{and}\quad
\mathrm{PA}\nvdash\neg\varphi.
\]
This is the derivability-based meaning of ``independent of PA'' used in this chapter. If consistency is not assumed, a \(C\)-condition for \(B_H^M(\bot)\) should receive the contradiction-dominance priority described in Theorem~\ref{thm:contradiction}.

\begin{theorem}[Unlimited-bound trichotomy for standard PA sentences]\label{thm:hypertrichotomy}
Let PA be consistent, let \(H\in M\) be unlimited, and let \(\varphi\) be a standard PA sentence. Exactly one of the following holds:
\begin{enumerate}[label=(\roman*)]
\item \(B_H^M(\varphi)\);
\item \(B_H^M(\neg\varphi)\);
\item neither \(B_H^M(\varphi)\) nor \(B_H^M(\neg\varphi)\).
\end{enumerate}
Moreover these cases correspond respectively to
\[
...
\end{theorem}
\begin{proof}
The equivalences are immediate from Theorem~\ref{thm:hyperproofequiv}. Consistency excludes simultaneous occurrence of the first two cases, because proofs of both \(\varphi\) and \(\neg\varphi\) yield inconsistency in ordinary classical PA. The third case is exactly the failure of both derivability statements.
\end{proof}

\begin{proposition}[Model-and-cutoff invariance on standard targets]\label{prop:hyperinvariance}
Let \(\N\preccurlyeq M_1\) and \(\N\preccurlyeq M_2\) be elementary extensions, and let \(H_1\in M_1\) and \(H_2\in M_2\) each be larger than every standard natural. For every standard PA sentence \(\varphi\),
\[
D_{H_1}^{M_1}(\varphi)=D_{H_2}^{M_2}(\varphi).
\]
Thus, on standard targets, the subscript and superscript record the idealization but do not change the classification.
\end{proposition}
\begin{proof}
By Theorem~\ref{thm:hyperproofequiv}, each bounded positive condition is equivalent to the same standard derivability statement \(\mathrm{PA}\vdash\varphi\) or \(\mathrm{PA}\vdash\neg\varphi\). Therefore the classification cannot depend on which elementary extension or which unlimited cutoff was chosen.
\end{proof}

\begin{warningbox}
The map \(D_H^M\) is a metatheoretically defined classification of standard syntax, not an effective procedure presented to a standard computer. In the full nonstandard-analysis reading, an internal hyperfinite scan \(E_H\) can represent the bounded search, but a standard observer is not thereby given a way to run to its last stage or read its result as ordinary algorithmic output. In the arithmetic-only reading, it is cleaner to regard \(B_H^M\) simply as a truth condition in the chosen elementary extension.
\end{warningbox}

\begin{corollary}[Idealized external settlement]\label{cor:hyperdecider}
Under the hypotheses of Theorem~\ref{thm:hypertrichotomy}, \(D_H^M\) assigns every standard PA sentence its correct \(P\), \(R\), or \(I\) metatheoretic case. If a \(C\)-condition is included and PA is inconsistent, every standard proof of contradiction is likewise below every unlimited cutoff.
\end{corollary}

This result does not contradict undecidability of ordinary computable PA theoremhood. Neither Compactness nor the bounded truth condition \(B_H^M\) supplies a standard program with a standard finite input and standard terminating runtime. The idealization uses a nonstandard bound together with the external/metatheoretic fact that the bound lies above every standard natural.
...
\end{theorem}
\begin{proof}
There are only finitely many natural numbers \(p\le N\), hence only finitely many proof codes at most \(N\), and therefore only finitely many theorem conclusions represented by those codes. PA has infinitely many distinct theorems; for example, the reflexive equalities for distinct numerals give infinitely many standard theorem statements. Consequently at least one PA theorem has no proof whose code is at most \(N\).
\end{proof}
...
Let \(H\in M\) be unlimited. There is no standard natural \(N\) that can replace \(H\) while preserving the property
\[
\text{every standard }n\in\N\text{ satisfies }n\le H.
\]
Consequently a bounded nonfinding statement such as
\[
M\models(\forall p\le H)\,\neg\Prf_{\mathrm{PA}}(p,\ulcorner\varphi\urcorner)
\]
cannot be turned into an equally exhaustive ordinary finite-search record merely by substituting a standard cutoff.
\end{proposition}
\begin{proof}
If a standard \(N\) dominated every standard natural, then the standard natural \(N+1\) would satisfy \(N+1>N\), a contradiction. Replacing \(H\) by any standard \(N\) therefore discards standard proof codes larger than \(N\).
\end{proof}

\begin{warningbox}
No ``standard-part'' operation repairs this problem. In a full hyperreal framework, standard part is defined for finite hyperreals, not as a natural-valued operation sending an unlimited hypernatural to an ordinary cutoff with the same cofinal property; see Goldblatt \cite{GoldblattHyperreals}. In the \(P\), \(R\), and \(C\) cases, Theorem~\ref{thm:hyperproofequiv} gives existence of an ordinary finite proof witness. In the \(I\) case, bounded nonfinding together with unlimitedness gives the external nonderivability conclusion, but it does not automatically produce a finite certificate in the AMLD's declared \(\mathcal K_I\) format.
\end{warningbox}

No conservativity or proof-translation theorem for a particular formalization of nonstandard analysis is assumed here. If some chosen formal NSA system is separately known to be conservative over, or interpretable in, an appropriate standard metatheory, that is an additional theorem about those formal systems. It should not be confused with replacing the particular nonstandard cutoff \(H\) by a standard finite cutoff.

\section{Three conclusions to keep separate}
One preliminary fact comes first: Compactness plus the elementary diagram supplies a proper elementary extension containing an unlimited nonstandard natural. Once such an environment is fixed, the search-bound argument establishes three further points:
\begin{enumerate}
\item \textbf{Coverage.} Every standard finite PA proof code lies below every unlimited cutoff.
\item \textbf{External classification.} Under elementarity and the stated consistency assumption, the bounded conditions give the correct external \(P/R/I\) classification of each standard PA sentence.
\item \textbf{Ordinary computability.} No ordinary algorithm deciding PA has been obtained. An unlimited \(H\) is not a standard finite resource bound, and the negative \(I\)-case is not automatically a standard finite AMLD certificate.
\end{enumerate}
The optional full-NSA picture adds a literal internal hyperfinite scan, but it does not alter any of these conclusions.

\begin{corollary}[Reconciliation with computational UNKNOWN]\label{cor:unknownhyper}
Corollary~\ref{cor:unknowncomputational} and Theorem~\ref{thm:hypertrichotomy} are compatible. An ordinary AMLD with a standard finite budget may return \textsf{UNKNOWN}. The nonstandard construction supplies an external classification theorem for standard PA sentences under the hypotheses above. It does not turn every external \(I\)-classification into a standard finite \(I\)-certificate, and it does not make PA decidable by an ordinary program.
\end{corollary}

\begin{exercise}[Building the unlimited horizon by compactness]\label{ex:compactness-unlimited}
Let \(\mathcal L(\N,H)\) be the language of arithmetic expanded by constants \(c_a\) for each \(a\in\N\) and one additional constant \(H\). Let
\[
T_H=\operatorname{ElDiag}(\N)\cup\{H>c_n:n\in\N\}.
\]
\begin{enumerate}[label=(\alph*)]
\item Explain why every finite subset of \(T_H\) is satisfiable in the standard natural numbers after choosing a sufficiently large ordinary interpretation for \(H\).
\item Apply Compactness to conclude that \(T_H\) has a model.
\item Explain why the named copy of \(\N\) embeds elementarily into that model and why the interpretation of \(H\) is outside the standard copy.
\item Explain why the definition of ``\(H\) is larger than every standard natural'' refers to the externally designated standard copy and is presented here as an infinite family of inequalities, not as an extra predicate added to PA.
\item Explain why the existence proof gives no ordinary finite algorithm that counts to \(H\).
\end{enumerate}
\end{exercise}

\begin{exercise}[Arithmetic core versus full NSA]\label{ex:twoframeworks}
Which parts of the chapter require only an elementary extension of arithmetic, and which phrases require the richer full nonstandard-analysis framework? In particular, classify each phrase: (a) \(B_H^M(\varphi)\); (b) ``internal hyperfinite interval''; (c) ``elementarity transfers the proof-existence statement''; (d) ``hyperfinite BANK history.''
\end{exercise}

\begin{exercise}[Why an arbitrary nonstandard PA model is not enough]\label{ex:arbitrary-pa-model}
Assume PA is consistent. Explain why a model of \(\mathrm{PA}+\neg\operatorname{Con}(\mathrm{PA})\) can contain a nonstandard number that the model regards as a PA proof code for contradiction. Which step of Theorem~\ref{thm:hyperproofequiv} fails if one works in this model instead of an elementary extension of the standard \(\N\)?
\end{exercise}

\begin{exercise}[Unlimited versus merely enormous]\label{ex:hyperbudget}
Let \(H\in M\) be unlimited and let \(N\in\N\) be an extremely large standard integer.
\begin{enumerate}[label=(\alph*)]
\item Why does mere membership \(H\in M\) not by itself state unlimitedness?
\item If PA proves a standard sentence \(\varphi\), why must \(B_H^M(\varphi)\) hold?
\item Why can no fixed standard \(N\) replace \(H\) in that universal coverage statement?
\item Explain the asymmetry between a positive \(P/R/C\) witness and the negative \(I\)-classification.
\item Why does the existence of the external classification \(D_H^M\) not contradict undecidability of ordinary computable PA theoremhood?
\end{enumerate}
\end{exercise}
...
Let $S$ be a finite set of candidate secrets and let the actual secret be a random variable $W\in S$. Let $Z$ denote all side information legitimately available to the analysis. For the realized side-information state $z$, assume the model supplies the conditional distribution
\[
\pi_z(w)=\Pr(W=w\mid Z=z).
\]
When $z$ is fixed, abbreviate $\pi_z$ by $\pi$. This is the search prior for the modeled episode. A secret generated uniformly and independently of $Z$ makes $\pi$ uniform on the generator's support. Human choice, reuse, policy constraints, previously disclosed information, or implementation artifacts may instead make $\pi$ highly nonuniform.
...
In this book, \emph{flood control} is an umbrella term for controls that bound or slow repeated authentication attempts. Standard security terminology includes \emph{rate limiting}, \emph{login throttling}, and \emph{account lockout}. If, within the explicitly modeled episode or time horizon, the system will process at most $N$ candidate submissions, we call $N$ the \emph{online attempt budget} for that episode.
\end{definition}

A delay schedule by itself does not automatically create a finite $N$: if the modeled horizon is unbounded, delayed attempts may continue indefinitely. The finite-attempt theorems below apply only when the episode definition, lockout rule, disabling rule, or finite time horizon supplies an actual or effective cap on the number of processed guesses.

For centrally verified passwords, NIST SP 800-63B-4 requires a rate-limiting mechanism. Unless an authenticator-specific rule says otherwise, its general rule limits consecutive failed attempts using a specific authenticator on one subscriber account to no more than 100 by disabling that authenticator; 100 is an upper bound, and lower limits are permitted \cite{NIST80063B4}. OWASP likewise recommends login throttling and discusses lockout thresholds, observation windows, lockout duration, and the denial-of-service risk of overly rigid lockout policies \cite{OWASPAuth}.

\section{The right quantity is top-\texorpdfstring{$N$}{N} probability mass}
Suppose the candidates are ordered so that
\[
\pi_{(1)}\ge \pi_{(2)}\ge\cdots\ge\pi_{(|S|)}.
\]
Define the \emph{top-$N$ exposure}
\[
G_N(\pi)=\sum_{i=1}^{\min(N,|S|)}\pi_{(i)}.
\]
...
If $W$ is uniform on $S$, then any such $N$-attempt online strategy succeeds with probability at most
\[
\min\left(1,\frac{N}{|S|}\right).
\]
\end{corollary}
...
The earlier distribution $\pi$ is already conditioned on the side information available for the episode. To avoid ambiguity with several inequivalent notions of conditional min-entropy used in information theory, define the min-entropy of this \emph{realized conditional distribution} by
\[
h_\infty(\pi)=-\log_2\left(\max_w \pi(w)\right).
\]

\begin{theorem}[Flood-control min-entropy bound]\label{thm:minentropy}
Under the hypotheses of Theorem~\ref{thm:topN},
\[
\Pr(\text{success in at most }N\text{ guesses}\mid Z=z)
\le
\min\left(1,N\,2^{-h_\infty(\pi)}\right),
\]
where $z$ is the fixed realized side information used to define $\pi$.
\end{theorem}
\begin{proof}
Every point probability is at most $2^{-h_\infty(\pi)}$. The probability mass of any $N$ guessed candidates is therefore at most $N$ times that quantity, capped by $1$.
\end{proof}
...
\begin{itemize}
\item A unique password drawn from a high-entropy generator can make $\pi$ close to flat over a large effective support, keeping $G_N(\pi)$ small for realistic $N$.
...
\item Password managers can generate and store long, unique secrets, reducing the human incentive to reuse memorable strings.
\end{itemize}
...
\begin{proposition}[Per-salt precomputation separation]\label{thm:salt}
Model a stored verifier as $y=F(s,w)$ where $s$ is an account-specific salt. Suppose a precomputed table contains only pairs of the form $(w,F(s_1,w))$. For a different salt $s_2$, that table does not by itself provide the values $F(s_2,w)$ unless one also has a method for deriving or computing the $s_2$-parameterized outputs. Thus a verifier for the second account must still use the second salt.
\end{proposition}
\begin{proof}
The table records outputs only for inputs whose first coordinate is $s_1$. An offline check for the second account requires outputs whose first coordinate is $s_2$. Without additional information or a derivation that supplies those outputs, the first table alone does not contain them.
\end{proof}

The proposition is intentionally modest. It does not claim that salting makes weak passwords safe or prevents per-account guessing. It states the narrower reason unique salts defeat direct cross-account reuse of a single one-salt password-to-hash lookup table.
...
If a system accepts only when both a password predicate $\mathsf{Pwd}$ and an additional-factor predicate $\mathsf{Factor}$ hold, then the set of fully accepted authentication events is a subset of the events satisfying $\mathsf{Pwd}$ alone. Consequently, success at the password subproblem is not sufficient for full authentication.
\end{theorem}
\begin{proof}
Every event satisfying $\mathsf{Pwd}\wedge\mathsf{Factor}$ satisfies $\mathsf{Pwd}$. The converse need not hold.
\end{proof}
...
Let $\mathsf{Req}(z)$ be the \emph{required/intended} acceptance predicate for a complete authentication event $z$, and let $\mathsf{Impl}(z)$ be the implemented acceptance predicate. For the security question of unintended acceptance, consider the one-sided conformance (acceptance-soundness) claim
\[
c_{\mathrm{auth}}:\qquad \forall z\,[\mathsf{Impl}(z)\Rightarrow \mathsf{Req}(z)].
\]
This says: every event the implementation accepts is an event the security specification intends to accept. It deliberately does \emph{not} say that every intended event is actually accepted. Full extensional conformance would additionally require the reverse implication, equivalently
\[
\forall z\,[\mathsf{Impl}(z)\leftrightarrow \mathsf{Req}(z)],
\]
and failures of that reverse direction represent false rejection, availability, or completeness defects rather than unintended acceptance. The one-sided target is used below because it isolates the safety property relevant to an unauthorized-acceptance witness. The mnemonic predicate names $\mathsf{Req}$ and $\mathsf{Impl}$ also avoid reusing the book's bare one-letter role symbols.
...
This one-sided conformance target gives the four AMLD roles concrete defensive meanings.
\begin{align*}
P &: \text{prove }c_{\mathrm{auth}}\text{ for the modeled state space};\\
R &: \text{produce a checked counterexample event }z^*;\\
I &: \text{certify, in the declared metatheory, that }c_{\mathrm{auth}}\text{ is independent of the admitted audit theory};\\
...
Suppose any one of routes $\rho_1,\ldots,\rho_k$ is sufficient to obtain the same protected account capability, and let $C_i$ be the minimum cost of successfully traversing route $\rho_i$ under a fixed threat model. Then the minimum cost of obtaining the capability satisfies
\[
...
\begin{proposition}[Authentication acceptance can be nonmonotone in time]\label{prop:authnonmono}
...
Suppose a password is uniformly random over a set $S$ with $|S|=2^{80}$ possibilities and the live verifier will process at most $N=100$ candidate submissions in the modeled episode. Corollary~\ref{cor:uniformN} gives
\[
...
Write a conformance statement $c$ for a toy authentication system in which acceptance is intended to require both a password predicate $\mathsf{Pwd}(z)$ and second-factor predicate $\mathsf{Factor}(z)$. What kind of object would let $R$ refute $c$?
\end{exercise}
...
record BankRecord:
    id                   # stable BANK record identifier
    claim
    semantic_kind        # AXIOM, HYPOTHESIS, INTERMEDIATE_THEOREM,
                         # TARGET_PROOF, NEGATED_TARGET_PROOF,
                         # INDEPENDENCE, CONTRADICTION, other declared kinds
    origin               # ENV, IMPORT, P, R, I, or C
    scope
    certificate
    dependency_ids
    verifier_receipt
    source_proposal_id   # NONE for ENV/IMPORT seed material
    source_future_node_id # NONE unless accepted from a FUTUREBANK node
    deposited_at
    environment_id

record Proposal:
    id                   # stable proposal identifier for history/audit
    submitting_role      # exactly one of P, R, I, C
    semantic_kind        # what kind of result is claimed
    claim                # formula/metaclaim claimed by the certificate
    certificate_type     # selects the formal checker
    certificate
    scope
    dependency_ids
    source_future_node_id # NONE unless materialized from a FUTUREBANK node
    environment_id
...
record FutureNode:
    id                   # stable FUTUREBANK history-node identifier
    proof_state
    parent_id            # NONE for root; otherwise one realized history parent
    generating_action
    role
...
The core pseudocode treats a \texttt{FutureNode} as a \emph{history node}: the root represents the current initial proof state and has no parent or generating action; every nonroot history node has one parent and one generating action. BANK records, proposals, and FUTUREBANK history nodes have stable identifiers because dependencies, history events, and invariant tests refer to them by ID. Every proposal-construction routine is required to allocate a fresh proposal ID. When SELECT materializes a FUTUREBANK history node, it marks that exact node as attempted and copies its ID into the proposal; if the proposal is accepted, UPDATE carries the same source-node ID into the resulting BANK record. BANK record identity is deliberately finer than formula identity: two separately accepted certificates may yield distinct BANK records even when their claims are syntactically identical. Retrieval indexes may coalesce or canonicalize equivalent claims for efficiency, but the underlying record ledger retains the separate receipts and provenance links. Two histories that reach the same visible proof state therefore remain distinct at this layer, so transition scores and provenance are unambiguous. A later implementation may share downstream computation across future-equivalent histories, but only through the safe quotient discipline of Chapter~\ref{ch:safequotient}; path multiplicity or ancestry that remains task-relevant must be retained separately.
...
function VERIFY(env, bank, proposal):
    ...
    # claim and scope are untrusted proposal fields until checked.
...
function UPDATE(state, proposal, result):
    ...
    # ACCEPT is returned only after the checker validates the semantic kind,
    # claim, scope, dependencies, certificate, and environment. The deposited
    # record copies fields that have therefore passed the declared checker.
    record = make_verified_record(proposal, result, state.step)
    record.id = fresh_record_id()
    record.source_proposal_id = proposal.id
    record.source_future_node_id = proposal.source_future_node_id
    state.bank.add(record)
    if proposal.source_future_node_id is not NONE:
        state.futurebank.mark_promoted(proposal.source_future_node_id,
...
function TERMINAL_STATUS(env, bank, accepted_record):
    # If inconsistency is already certified, it dominates in explosive logic.
    if bank.has_verified_contradiction(env):
        return INCONSISTENT
    # Otherwise classify this accepted terminal record by verified semantics,
    # not by who found it.
...
The pseudocode assumes explosive logic and gives an already verified contradiction certificate priority over other terminal evidence. This is an operational precedence rule, not clairvoyance: if a proof of $c$ is accepted and the run halts before any contradiction is discovered, the machine cannot retroactively know that a future $C$-search would have succeeded. In a different logic, terminal precedence belongs in the verification environment. The classifier deliberately does not test the record's origin. If $R$ submits an accepted target-proof certificate, the provenance remains $R$ while the semantic status is \textsf{PROVED}. Also, first-certificate reporting should not be confused with a proof that the four evidence conditions are globally exclusive; Corollary~\ref{cor:overlap} and Theorem~\ref{thm:statuspartition} state the relevant distinction.
...
function IMAGINE(state, depth_budget, branch_budget):
    ...
    while not frontier.is_empty() and created < branch_budget:
        node = frontier.pop()
        if node.depth >= depth_budget:
            continue

        # Each generated action carries the role that generated it.
        actions = GENERATE_DEDUCTIVE_ACTIONS(state, node)

        for action in actions:
            if created >= branch_budget:
                break

            child = APPLY_COUNTERFACTUALLY(node, action)
            local_result = LOCAL_CHECK(action, node, child)
            if local_result == PASS:
                child.trust_grade = LOCAL
            elif action.is_allowed_as_explicit_hypothesis():
                child.trust_grade = SPEC
            else:
                continue  # malformed or disallowed transitions are not branches

            child.id = fresh_id()
            child.parent_id = node.id
            child.generating_action = action
            child.role = action.role
            child.depth = node.depth + 1
            child.bank_dependencies = trace_bank_dependencies(action)
            child.assumed_dependencies = trace_hypotheses(action)
            child.estimated_cost = estimate_cost(state, node, action)
...
function COMPASS(state):
    # K1 scores transitions, so the root (which has no incoming transition)
    # is not assigned a K1 edge score here.
    for node in state.futurebank.rankable_nonroot_nodes():
        parent = state.futurebank.parent_proof_state(node)  # unique history parent
        node.compass_score = K1(parent, node.proof_state)
...
function SELECT(state, controller_action, fallback=None):
    if controller_action.kind == "TRY_FUTURE_NODE":
        node = controller_action.node
        node.attempted = true
        proposal = materialize_certificate_proposal(node)
        proposal.source_future_node_id = node.id
        return proposal
...
function AMLD_DECIDE(env, resource_budget):
    B0 = SEED_BANK(env)
    x0 = initial_proof_state(env, B0)
    F0 = new FutureBank(root = FutureNode(
        id = fresh_id(),
        proof_state = x0,
        parent_id = NONE,
        generating_action = NONE,
        role = NONE,
        assumed_dependencies = {},
        bank_dependencies = ids(B0),
        depth = 0,
        trust_grade = LOCAL
    ))
    ...
    while resource_budget.allows_next_step():
        ...
        if fallback is not NONE and \
           is_reserved_fallback_step(state.step, state.controller_state):
            # Required to return the same typed Proposal used by all search paths.
        ...
        state, accepted_record = UPDATE(state, proposal, result)

        # 6. Classify the certificate that was just accepted. Do not infer
        # terminal semantics from whichever retrieval/index entry changed last.
        if accepted_record is not NONE:
            status = TERMINAL_STATUS(env, state.bank, accepted_record)
            if status != NONE:
                return status, state
...
A fallback engine is a search component, not a fifth semantic role. Its \texttt{next\_proposal} interface is required to return the same typed \texttt{Proposal} record as every other search path, including one submitting-role tag, a declared semantic certificate kind, an explicit claim and scope, and the certificate body. Fallback therefore has no separate route around \texttt{VERIFY}.

\chapter{Safe compression and quotienting}\label{ch:safequotient}
\begin{miniroadmap}
...
Let $Q(h)$ be a compressed descriptor of FUTUREBANK history $h$.

\begin{definition}[Future-sufficient descriptor]
A descriptor $Q$ is \emph{future-sufficient} for a declared task if
\[
Q(h_1)=Q(h_2)
\]
...
If $Q$ is future-sufficient and any task-relevant path multiplicity or ancestry is retained separately, then histories with equal $Q$-state may share downstream computation without changing the represented continuation structure relevant to that task.
\end{proposition}
\begin{proof}
By future-sufficiency, equality of descriptors implies equality of all continuation information that the declared task uses for future search, verification, scope, and counting/provenance semantics. Sharing the downstream computation therefore identifies only continuations that the task already treats as equivalent. Any path information that the task still distinguishes is preserved separately by hypothesis.
\end{proof}
...
property futurebank_promotion_is_verifier_gated:
    for r in bank.derived_records:
        if r.source_future_node_id is not NONE:
            assert r.verifier_receipt is not NONE
            assert futurebank.promotion_of(r.source_future_node_id) == r.id
...
These properties are part of what it means to make the architecture scientific. They turn slogans into falsifiable software claims. In particular, FUTUREBANK membership itself is never compared with BANK membership as though the two stored the same record type; promotion is tested through the explicit source-node/proposal/receipt chain.
...
\[
D(c)\in\{\textsf{PROVED},\textsf{REFUTED},\textsf{INDEPENDENT},\textsf{INCONSISTENT},\textsf{UNKNOWN}\}
\]
for the operational status returned by the AMLD on a PA well-formed formula $c$. The letters $P,R,I,C$ name search roles and designated settlement roles; they are not themselves status values. Thus a verified negated-target proof has designated settlement role $R$ and licenses status \textsf{REFUTED}, regardless of which role submitted it. Under the contradiction-dominance convention, an accepted contradiction certificate licenses \textsf{INCONSISTENT} with priority in an explosive inconsistent background. \textsf{UNKNOWN} remains an operational resource outcome rather than a fifth logical truth class.
...
\Hist_0=\varnothing,
\]
where $h_0$ is the length-zero root history whose endpoint is the initial proof-search state induced by $\Bank_0$. No counterfactual branch has yet been added. We now follow one cycle.
...
\[
\boxed{D(c)=\textsf{REFUTED}\qquad(\text{designated settlement role }R).}
\]
...
One suitable one-sided safety target is $c:\forall z[\mathsf{Impl}(z)\Rightarrow(\mathsf{Pwd}(z)\wedge\mathsf{Factor}(z))]$. The refuter $R$ needs a checked event $z^*$ for which the implementation accepts but at least one required predicate is false. Such a witness is a finite counterexample certificate. A separate reverse implication would be needed if the exercise also required complete acceptance of all intended events.
...
\paragraph{Exercise~\ref{ex:compactness-unlimited}.}
(a) A finite subset mentions only finitely many inequalities $H>c_{n_i}$, so the standard model satisfies that subset when $H$ is interpreted by any ordinary $m$ larger than all listed $n_i$. (b) Compactness gives a model of the whole theory. (c) A model of $\operatorname{ElDiag}(\N)$ yields an elementary embedding of the named standard structure; the added inequalities force $H$ to lie above every named standard natural and hence outside that embedded copy. (d) ``standard'' refers to the externally designated embedded copy of $\N$; the construction expresses unlimitedness by the external scheme $H>c_n$ for each standard $n$, without adding a special ``standardness'' predicate to PA. (e) Model existence is not an effective presentation of the model or a finite runtime bound.

\paragraph{Exercise~\ref{ex:twoframeworks}.}
(a) $B_H^M(\varphi)$ uses only the arithmetic elementary-extension framework. (b) ``Internal hyperfinite interval'' belongs to the richer full-NSA framework. (c) Elementarity of the arithmetic extension is enough. (d) A literal internal hyperfinite BANK history again requires a full nonstandard enlargement carrying the relevant state objects, not merely a nonstandard model of arithmetic.

\paragraph{Exercise~\ref{ex:arbitrary-pa-model}.}
By the second incompleteness theorem, consistent PA does not prove its own usual consistency sentence. Therefore $\mathrm{PA}+\neg\operatorname{Con}(\mathrm{PA})$ is consistent and has a model $A$. In $A$, $\neg\operatorname{Con}(\mathrm{PA})$ says that some $p$ satisfies the arithmetized PA proof predicate for contradiction. Externally no standard $p$ can do so, hence the witness is nonstandard. The failed step is the appeal from $A\models\exists p\,\Prf_{\mathrm{PA}}(p,\ulcorner\bot\urcorner)$ back to the standard structure $\N$: $\N$ is not assumed elementary in this arbitrary model. Unlimitedness of the witness does not repair that failure.

\paragraph{Exercise~\ref{ex:hyperbudget}.}
(a) $M$ contains the embedded standard naturals as well as nonstandard elements, so membership $H\in M$ alone says nothing about whether $H$ dominates every standard natural. (b) Any ordinary PA proof has a standard finite code $n$, and unlimited $H$ satisfies $n<H$, so that same code witnesses $B_H^M(\varphi)$. (c) No standard $N$ dominates all standard naturals; in particular $N+1>N$. Theorem~\ref{thm:finitecutoff} sharpens this by showing that some standard PA theorem has no proof code below any prescribed finite cutoff. (d) A positive bounded proof witness implies by elementarity that an ordinary finite proof exists. In the $I$ case, failure of both bounded proof-existence conditions yields an external nonderivability classification, not automatically a finite independence certificate. (e) $D_H^M$ is a metatheoretically defined classification using a nonstandard bound, not a standard algorithm with a standard finite runtime.
...
\item a statistical decision rule that distinguishes ``insufficient evidence'' from evidence that a prespecified effect is smaller than $\delta$.
\end{itemize}
...
\textbf{Refutation target.} Preregister a benchmark-relative claim such as a minimum $\delta$ reduction in verifier calls together with a one-sided confidence-bound or equivalence-testing rule capable of excluding an improvement of that size. Refute that finite claim when the preregistered analysis excludes an improvement of size $\delta$ (or supports a practically meaningful effect in the wrong direction). Merely failing to obtain statistical significance is not, by itself, evidence that the claimed advantage is absent.
...
\textbf{Refutation target.} Use the preregistered decision rule to show that speculative expansion falls short of the declared efficiency threshold, or that memory/branch explosion increases total cost enough to erase the benefit.
...
\textbf{Refutation target.} On a preregistered deceptive-guidance benchmark, use the declared decision rule to show that the tension signal fails to achieve the stated improvement, or that it systematically triggers escapes just before the local search would have succeeded.
...
\textbf{Refutation target.} Under the preregistered resource metric, show that speculative downstream analysis costs more than the proof effort it saves, or that it repeatedly selects high-utility-looking lemmas that turn out to be impossible to backfill.
...
\textbf{Ablation.} Implement at least three reflection budgets: none, moderate, and high. If using the Level terminology, implement the declared self-description schemas explicitly and charge their construction/checking cost rather than treating a level label as free metadata.

\textbf{Refutation target.} Refute a benchmark-specific ``middle is best'' claim under the preregistered decision rule by establishing monotone improvement with more reflection, monotone degradation, or equivalence within the declared practical margin. To attack the stronger slogan ``Level 2 is universally optimal,'' a single well-specified architecture and benchmark in which another declared level strictly wins is enough.
...
\textbf{Refutation target.} Reject the benchmark-level advantage claim when the preregistered analysis excludes the declared value improvement for condition A---for example, because verified defect/invariant yield is equivalent within the practical margin or because overhead erases the claimed gain.
...
\textbf{Refutation target.} Use the preregistered decision rule to establish that sharing provides less than the declared practical audit benefit, or that bookkeeping and overly broad reusable facts create enough overhead to make isolation better.
...
\textbf{Refutation target.} Plot unauthorized-acceptance risk against legitimate-user denial and latency. Refute the finite benchmark claim if the preregistered analysis establishes that the adaptive policy is Pareto-dominated by a fixed policy or falls short of the declared improvement margin.
...
\textbf{Refutation target.} Reject the advantage claim when the preregistered analysis excludes the declared improvement in verified repair rate, or when history-driven search produces a practically meaningful increase in wasted budget on stale failure modes.
...
\markboth{Closing roadmap}{Closing roadmap}
...
\item The verifier is the fixed gate between ``this might work'' and ``this is now an accepted formal conclusion in the declared environment.''
\end{enumerate}
...
Snapshot repository: \href{https://github.com/btenneson/Automatic-Logical-Deciding-Snapshot-August-2-2026-}{https://github.com/btenneson/Automatic-Logical-Deciding-Snapshot-August-2-2026-/}.\ \href{https://btenneson.github.io/pub/papers/ald_certificate_halting_shared_lemma_pools/}{Live article}.
\bibitem{TennesonDepths2026}
Brian Tenneson,
\emph{Depths of a Simulation \& Formalized Self-Awareness}, merged edition, July 2026. Author manuscript. Public repository landing page: \href{https://github.com/btenneson/Depths-of-a-Simulation}{https://github.com/btenneson/Depths-of-a-Simulation}.
...
\bibitem{EndertonLogic}
Herbert B. Enderton,
\emph{A Mathematical Introduction to Logic}, 2nd ed., Academic Press, 2001.
See Section 2.5 for first-order Soundness, Completeness, and Compactness, Section 2.8 for nonstandard analysis, and Section 3.7 for arithmetized provability and the Second Incompleteness Theorem.
DOI: \href{https://doi.org/10.1016/C2009-0-22107-6}{https://doi.org/10.1016/C2009-0-22107-6}.\\
Publisher page: \href{https://www.sciencedirect.com/book/9780122384523/a-mathematical-introduction-to-logic}{ScienceDirect}.

\bibitem{MarkerModelTheory}
David Marker,
\emph{Model Theory: An Introduction}, Graduate Texts in Mathematics 217, Springer, 2002.
See Section 2.1 for Compactness and Section 2.3 (beginning on p.~44) for elementary embeddings, diagrams, and elementary extensions.
DOI: \href{https://doi.org/10.1007/b98860}{https://doi.org/10.1007/b98860}.
...
\bibitem{GoldblattHyperreals}
Robert Goldblatt,
\emph{Lectures on the Hyperreals: An Introduction to Nonstandard Analysis}, Graduate Texts in Mathematics 188, Springer, 1998.
See pp.~35--58 for transfer and infinite/infinitesimal hyperreals, pp.~125--154 for internal/external and hyperfinite sets, and pp.~157--189 for nonstandard frameworks and existence.
DOI: \href{https://doi.org/10.1007/978-1-4612-0615-6}{https://doi.org/10.1007/978-1-4612-0615-6}.

\bibitem{SEPProvability}
Rineke Verbrugge,
``Provability Logic,'' \emph{The Stanford Encyclopedia of Philosophy}, substantive revision 2024.
Live entry: \href{https://plato.stanford.edu/entries/logic-provability/}{https://plato.stanford.edu/entries/logic-provability/}.

\end{thebibliography}

\chapter*{Editorial provenance note}
\markboth{Editorial provenance note}{Editorial provenance note}
\addcontentsline{toc}{chapter}{Editorial provenance note}
The AMLD roles, shared BANK, provenance emphasis, FUTUREBANK, compass/control architecture, verifier separation, and formalized self-description program are developed from Brian Tenneson's prior manuscripts and research discussions. Versions 1.01--1.03 introduced and progressively qualified the compactness-based nonstandard discussion. Version 1.04 performs a stabilization audit of the entire nonstandard-analysis chapter: it makes the arithmetic elementary-extension framework primary, quarantines literal internal/hyperfinite language to a separately identified full-NSA framework, replaces process-like claims about an externally observable completed search with a bounded truth condition, proves model-and-cutoff invariance on standard targets, qualifies standard-part language, adds Goldblatt as a dedicated NSA reference, and supplies solution sketches for the new NSA exercises. Version 1.05 makes the framework separation syntactic as well as verbal: the compactness-built arithmetic extension is denoted $M$, the star notation ${}^*\N$ is reserved for an optional full NSA enlargement, and the bounded condition/classifier are written $B_H^M$ and $D_H^M$. This removes the last systematic ambiguity between model-theoretic nonstandard arithmetic and full internal/hyperfinite NSA. Version 1.06 makes explicit the strongest hidden hypothesis: the reverse implication from a nonstandard proof witness to an ordinary proof depends on $\N\preccurlyeq M$, not merely $M\models\mathrm{PA}$. A G\"odel-II counterexample shows why arbitrary nonstandard PA models are insufficient, and the text now states that the full elementary diagram is a non-effective semantic resource rather than a computational recipe. Version 1.07 makes explicit that the compactness-built extension satisfies $M\models\operatorname{Th}(\N)$, not merely PA, and repairs the last dropped ambient-model superscript in the bounded-condition notation. Version 1.08 defines the elementary-extension notation at first use, makes the ambient membership $H\in M$ explicit in the trichotomy theorem, and spells out the initial-segment fact used in the G\"odel-II counterexample. Version 1.09 is the solution-sketch fixed-point pass: it removes the last residual ${}^*\N$ notation from the arithmetic-only answer to Exercise 17.19(a), replacing it by the ambient elementary extension $M$. The chapter's fixed correctness claims are now stated entirely in terms of standard PA syntax, elementary extensions, unlimited bounds, and metatheoretic classification; no theorem depends on treating a nonstandard bound as an executable finite resource. The book was substantially reorganized, expanded, formalized, technically audited, and edited with AI assistance across the v1.0--v1.09 development. Version 1.49 is the first post-NSA stable-release audit: Chapter~17 and its NSA solution sketches are held fixed, while the remaining text is checked for mathematical, architectural, statistical, notation, pseudocode, cross-reference, citation, and presentation defects. Version 1.89 is the second stable audit, resolving remaining global notation collisions, defining the full AMLD state space explicitly, sharpening the finite-attempt authentication model, aligning the illustrative pseudocode with the mathematical interfaces, and tightening empirical-refutation language. Version 2.29 repairs the remaining role/status conflation in the worked PA run and corrects a residual quotation defect, while continuing to hold the nonstandard-analysis material fixed. Version 2.69 repairs the executable interface contract by making proposal claims and scopes explicit checker inputs, binding accepted receipts to those checked fields, adding the FUTUREBANK node identifier used by the pseudocode, and requiring fallback to return the same typed proposal object as every other search path. Version 3.09 removes two residual overstatements: it distinguishes environment-admitted seed assumptions from verifier-gated derived deposits, states backfill as a sufficient rather than necessary promotion condition, and sharpens the benchmark-refutation testing language. Version 3.49 distinguishes the authentication chapter's one-sided unauthorized-acceptance safety property from full extensional implementation/specification conformance, preventing a rejecting-all implementation from being described as fully conformant. Version 3.89 resolves a FUTUREBANK representation mismatch: transition scores and generating actions are now attached to single-parent history nodes in the core pseudocode, while DAG-style computation sharing is deferred to the separately proved safe-quotient layer. Version 4.29 repairs FUTUREBANK initialization and root handling: the full AMLD loop now seeds an explicit root history node from the initial BANK/proof state, and first-order transition scoring excludes that parentless root. Version 4.69 completes the pseudocode identity and budgeting contract: BANK records and proposals receive stable IDs, newly imagined history nodes receive their ID/parent/action/role/depth metadata explicitly, and IMAGINE now visibly enforces both depth and branch limits rather than referring to an unconsumed abstract budget. Version 5.09 repairs provenance identity across namespaces: derived BANK records now retain their source proposal ID, FUTUREBANK-materialized proposals retain their source node ID, rejected-proposal invariants test that actual link, and accepted promotion is recorded against the exact source future node. Version 5.49 fixes the remaining record-identity ambiguity by separating immutable BANK ledger records from formula-level retrieval deduplication, so repeated proofs can share lookup structure without erasing accepted receipts or provenance. Version 5.89 repairs the FUTUREBANK promotion invariant: accepted BANK records now retain the source FUTUREBANK node ID, SELECT explicitly marks and links attempted future nodes, and the executable test follows that provenance chain instead of comparing unlike FutureNode and BankRecord objects. Version 6.29 removes remaining cross-chapter notation collisions in the authentication example, aligns the worked PA run with the nonempty root-history definition of FUTUREBANK, and makes the seed exercise's two-deposit/three-state request internally consistent. Version 6.30 is a document fixed-point pass: it adds the author credentials, contact information, copyright notice, explicit page-number footer, and running-head apparatus; audits citation keys against bibliography entries; verifies the principal live reference targets; makes the previously uncited program-context and Keisler references explicit in the text; and rechecks LaTeX compilation, cross-references, bibliography resolution, and rendered-page layout until a subsequent audit pass produces no further source changes. Version 6.40 adds a plain-language elementary description to the title page so that the core distinction between imaginative search and verified mathematical acceptance is visible before any technical terminology is assumed. Version 6.80 expands that title-page description to a longer elementary blurb and adds a separate elementary foreword designed to give a non-specialist reader a two-to-three-page conceptual bridge into the BANK, FUTUREBANK, four settlement roles, verifier boundary, UNKNOWN output, COMPASS-guided search, provenance, and formalized self-description before the technical foreword begins. Because the scope and prose have changed considerably from earlier preview drafts, attribution percentages stated in those earlier drafts should not be assumed to apply unchanged to this edition.
