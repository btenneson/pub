...
HYPERFINITE_HREFS = {
    "papers/hyperfinite_oracles_data_mind_3_2/",
    "papers/hyperfinite_oracles_data_mind_3_2/The_Hyperfinite_Oracles_of_DATA_MIND_3_2.pdf",
}
...
def detach_one(items: list[dict], hrefs: set[str], label: str) -> dict:
    matches = [x for x in items if str(x.get("href") or "") in hrefs]
    if len(matches) != 1:
...
def main() -> None:
    ...
    hyperfinite = detach_one(items, HYPERFINITE_HREFS, "hyperfinite-oracles")
    ...
    print("Hyperfinite Oracles target:", hyperfinite.get("href"))
