...
\downarrow\\[2pt]
\text{discovery-sensitive intervention control}\\[2pt]
\downarrow\\[2pt]
\text{Abel/Bellman progress estimation and audited iteration}\\[2pt]
\downarrow\\[2pt]
\text{certificate-bearing fixed point.}
\end{array}
\]
The strongest claim is not that the fixed point becomes easy. It is that each justified transformation can remove a class of states or routes that do not need to be searched for the declared objective, while the discovery layer can learn which surviving transformation family is worth trying next and the Abel layer can measure graded motion when the binary endpoint is too coarse.
...
\begin{conjecture}[Transferable approximate Abel coordinate]
There exist nontrivial theorem families for which an Abel/Bellman coordinate learned only from earlier audited rungs has positive out-of-sample discovery drift, bounded residual, and sufficient action-gap accuracy to predict useful intervention choices on later sealed rungs better than the ordinary-search baseline.
\end{conjecture}

\begin{conjecture}[Discovery-control escalation]
If a discovery controller is genuinely learning mechanism rather than one-family syntax, then its prospectively measured intervention advantage should survive at least one sealed cross-family replication after the feature map, action grammar, and controller have been frozen.
\end{conjecture}

Key implementation questions include whether cumulative-history canonicalization is cheap enough to pay for itself; how to define rational edge costs that remain meaningful across machines; how much of proof-state equivalence can be checked exactly; whether macro compilation should affect search cost, native proof cost, or both; which meta-theorems are strong enough to improve scheduling rather than merely restating architecture; whether a low-dimensional Abel coordinate exists on discovery trajectories; when scalar drift must be replaced by vector or history-valued state; and whether a system can learn, from earlier settled rungs, when a qualitatively different mathematical mechanism is becoming more valuable than continued local search.
